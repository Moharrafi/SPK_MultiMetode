...
export default function PengaturanPage() {
    ...
    if (file) {
      ...
      void uploadImage(file)
        .then((url) => {
          ...
          setLogoPreview(url)
          setLocalReportSettings((prev) => ({ ...prev, logoUrl: url }))
          updateReportSettings({ logoUrl: url })
          showToast(
            language === "id" ? "Logo laporan disimpan otomatis" : "Report logo saved automatically",
            "success",
          )
        })
        .catch((error) => {
  ...
  const handleRemoveLogo = () => {
    setLogoPreview("")
    setLocalReportSettings((prev) => ({ ...prev, logoUrl: "" }))
    updateReportSettings({ logoUrl: "" })
  ...
  const handleAppLogoUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    void uploadImage(file)
      .then((url) => {
        URL.revokeObjectURL(previewUrl)
        setLocalAppLogo(url)
        updateAppSettings({ appLogoUrl: url })
